...
static int cbp72_edpram_wait_resp(struct dpram_link_device *dpld, u32 resp)
{
	int ret;
	int int2cp;

	ret = wait_for_completion_interruptible_timeout(
			&dpld->udl_cmpl, UDL_TIMEOUT);
	if (!ret) {
...
static int cbp72_edpram_download_bin(struct dpram_link_device *dpld,
			struct sk_buff *skb)
{
	...
	if (bf->resp) {
		err = cbp72_edpram_wait_resp(dpld, bf->resp);
		if (err < 0) {
...
static int cbp72_xmit_binary(struct dpram_link_device *dpld,
			struct sk_buff *skb)
{
	if (dpld->type == EXT_DPRAM)
		return cbp72_edpram_download_bin(dpld, skb);
	else
		return -ENODEV;
}
...
static int cbp72_edpram_upload(struct dpram_link_device *dpld,
		struct dpram_dump_arg *dump, unsigned char __user *target)
{
	...
	if (dump->resp) {
		err = cbp72_edpram_wait_resp(dpld, dump->resp);
		if (err < 0) {
			mif_info("ERR! wait_response fail (err %d)\n", err);
			goto exit;
		}
	...
	memcpy(buff, dest + sizeof(struct dpram_udl_header), plen);
...
static int cbp72_dump_update(struct dpram_link_device *dpld, unsigned long arg)
{
	...
	if (ret  < 0) {
		mif_info("ERR! copy_from_user fail (err %d)\n", ret);
		return ret;
	}

	return cbp72_edpram_upload(dpld, &dump, (unsigned char __user *)arg);
}
#endif
...
static int cmc221_xmit_boot(struct dpram_link_device *dpld, unsigned long arg)
{
	...
	if (cp_img.mode == HOST_BOOT_MODE_NORMAL) {
		mif_info("HOST_BOOT_MODE_NORMAL\n");
			...
			struct modem_firmware *fw)
{
	...
	while (rest > 0) {
		...
		if (curr_frame == 1) {
			...
			dpld->send_intr(dpld, CMD_IMG_SEND_REQ);
	...
	mif_err("send CMD_DL_SEND_DONE_REQ to CP\n");
	dpld->send_intr(dpld, CMD_DL_SEND_DONE_REQ);
