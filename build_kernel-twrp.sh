...
## a 3.* config exists??? Clean the Tree as we are doing a release build
if [ -f $KERNELDIR/.config ];
then
  grep 'DreamKernel-v2.' $KERNELDIR/.config > /dev/null
  if [ "$?" == "0" ];
  then
    echo -e "${TXTYLW}found a 2.x series config ..., cleaning kernel Source tree${TXTCLR}"
    make -j10 distclean
  else
    echo -e "${BLDGRN}Found a 3.x Series config OK....${TXTCLR}"
  fi
fi
